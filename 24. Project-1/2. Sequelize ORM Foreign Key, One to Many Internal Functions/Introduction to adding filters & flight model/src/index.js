...
app.listen(ServerConfig.PORT, async () => {
  ...
  // Basic Testing Code - BAD CODE ALERT !

  const { City, Airport } = require("./models");
  const city = await City.findByPk(7);
  console.log(city);
  // const res1 = await city.createAirport({ name: "CSI airport", code: "CSI" });
  // const ans2 = await city.createAirport({ name: "CBI airport", code: "CBI" });
  // await City.destroy({ // All the columns associated with the [city with id = 7] in another table Airports should also be deleted if the city with id = 7 is deleted in the City table.
  //   where: {
  //     id: 7,
  //   },
  // });
